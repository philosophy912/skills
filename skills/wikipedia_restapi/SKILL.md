...
description: 通过 Wikipedia / Wikimedia REST API 查询条目、搜索、算公式、读页面 HTML、读数学公式。覆盖 Special:RestSandbox 中 6 个 API 集合（145 个 endpoint），统一通过 `scripts/wikipedia_api.sh` 用 curl 访问，代理和语言通过 `~/.wikipedia_restapi.json` 配置文件管理（首次运行 `setup_config.sh` 引导式生成）。当用户要查维基百科、搜条目、读页面摘要 / HTML、读 / 检查 LaTeX 公式、查 OpenSearch 描述、查站点 / 条目 attribution 信息时使用此 skill。
---

# wikipedia_restapi

通过 Wikimedia 官方 REST API 访问维基百科内容。所有调用统一走 `scripts/wikipedia_api.sh`，使用 curl 直连。
...
```
${CLAUDE_PLUGIN_ROOT}/skills/wikipedia_restapi/scripts/wikipedia_api.sh
```

> 下文为简洁起见，用 **`WIKI`** 代表
> `bash "${CLAUDE_PLUGIN_ROOT}/skills/wikipedia_restapi/scripts/wikipedia_api.sh"`。
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/skills/wikipedia_restapi/scripts/setup_config.sh"
```
...
- `<endpoint-name>` 短名（见 `references/endpoints.md` 或 `wikipedia_api.sh` 内 `endpoint_path` 函数）
...
|---------|------|:---:|-------|
| `wmf-restbase` | Wikimedia REST API（页面、阅读列表、推荐、Transform） | ✅ | 46 |
| `wmf-restbase-global` | Math API（wikimedia.org，跨语种） | ❌ | 3 |
| `mw-extra` | MediaWiki REST API（OAuth、CheckUser、CampaignEvents、Page v1、Search） | ✅ | 68 |
| `growthexperiments.v0` | Growth experiments API（新手任务、User impact、Suggestions） | ✅ | 9 |
...
- `scripts/wikipedia_api.sh` — 主入口脚本（封装 145 个 endpoint 的 curl 命令）
- `scripts/setup_config.sh` — 引导式配置生成器（一次运行，写入 `~/.wikipedia_restapi.json`）
- `references/endpoints.md` — 完整 endpoint 表（145 行，方法 / 路径 / 摘要）
...
2. 更新 `scripts/wikipedia_api.sh` 中 `endpoint_path` 函数的 case 表。
