...
本 skill 随 `philosophy` 插件分发。所有子命令通过包装脚本 `catva` 调用；
`${CLAUDE_PLUGIN_ROOT}` 会被 Claude Code 自动替换为插件安装目录的绝对路径：

```bash
bash "${CLAUDE_PLUGIN_ROOT}/skills/cat-video-analyzer/scripts/catva" <子命令> [...]
```

> 下文为简洁起见，用 **`CATVA`** 代表上面这整条命令（即
> `bash "${CLAUDE_PLUGIN_ROOT}/skills/cat-video-analyzer/scripts/catva"`）。
...
下表中 **`CATVA`** = `bash "${CLAUDE_PLUGIN_ROOT}/skills/cat-video-analyzer/scripts/catva"`。
...
- Python ≥ 3.10
- `ffmpeg`（系统级，PATH 中可用；脚本会自动探测常见安装位置）
