...
输出 JSON，逐项报告 `python` / `ffmpeg` / `config` 的就绪状态。任一不通过时按返回值处理：

- **ffmpeg 缺失** → 返回值里带 `install_hint`，给出当前平台的安装命令
  （macOS `brew install ffmpeg` / Linux `apt install ffmpeg` / Windows `winget install Gyan.FFmpeg`）。
  ffmpeg 是**系统级依赖，本 skill 不会自动安装**，请提示用户手动装一次（装完即在 PATH 中）。
- **config 未填** → 编辑返回的 `config.path`（`~/.config/cat-video-analyzer/config.toml`），
  填入 NAS 上猫砂盆与喂食机两个视频目录。

`doctor` 全部 `ok` 才适合进入下面的工作流（退出码 0 = 就绪，1 = 有缺失）。
...
| `CATVA doctor` | 检查 Python / ffmpeg / 配置是否就绪 |
